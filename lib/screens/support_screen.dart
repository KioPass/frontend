...
class _SupportScreenState extends State<SupportScreen> {
  ...
  List<InquiryModel> _myInquiries = [];
  ...
  Future<void> _deleteInquiry(InquiryModel inq) async {
    ...
    setState(() => _deletingIds.add(inq.id));
    final ok = await ApiService.deleteInquiry(token, inq.id);
    if (!mounted) return;

    setState(() => _deletingIds.remove(inq.id));
    if (ok) {
  ...
  Future<void> _loadMyInquiries() async {
    final token = await AuthService.getToken();
    if (token == null) {
      setState(() => _inquiriesLoading = false);
      return;
    }
    final list = await ApiService.getMyInquiries(token);
    if (mounted) setState(() { _myInquiries = list; _inquiriesLoading = false; });
  }
  ...
  Future<void> _submitInquiry() async {
    if (_selectedCategory == null || _inquiryController.text.trim().isEmpty) return;
    ...
    final ok = await ApiService.createInquiry(token, _selectedCategory!, _inquiryController.text.trim());
    if (!mounted) return;

    if (ok) {
      setState(() { _isSubmitted = true; _isLoading = false; });
      await _loadMyInquiries();
    } else {
      setState(() => _isLoading = false);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('문의 접수에 실패했어요. 다시 시도해주세요.')),
      );
    }
  }
          ...
          SliverToBoxAdapter(
            child: Column(
              ...
              children: [
                ...
                else if (_myInquiries.isNotEmpty)
                  Container(
                    color: cs.surface,
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Container(
                                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                decoration: BoxDecoration(color: cs.onSurface.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(6)),
                                child: Text('${_myInquiries.length}', style: TextStyle(fontFamily: 'Pretendard', color: cs.onSurface.withValues(alpha: 0.6), fontSize: 11, fontWeight: FontWeight.w700)),
                              ),
                            ],
                          ),
                        ),
                              ...
                              Theme(
                                ...
                                child: ExpansionTile(
                                  ...
                                  subtitle: Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        Text(inq.createdAt, style: tt.bodySmall),
                                        const SizedBox(width: 8),
                                        Container(
                                          padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                          decoration: BoxDecoration(
                                        ...
                                        const Spacer(),
                                        _deletingIds.contains(inq.id)
                                            ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2))
                                            : GestureDetector(
                                                onTap: () => _deleteInquiry(inq),
                                                child: Icon(Icons.delete_outline_rounded, size: 18, color: cs.onSurface.withValues(alpha: 0.35)),
                                              ),
                                      ],
                                    ),
                                  ),
                                  children: isDone && inq.answer != null
                                      ? [
                                          Container(
                      ...
                      if (_isSubmitted)
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed: _selectedCategory != null && _inquiryController.text.trim().isNotEmpty && !_isLoading
                                ? _submitInquiry
                                : null,
                            ...
                            child: _isLoading
                                ...
                                : const Text('문의 접수하기', style: TextStyle(fontFamily: 'Pretendard', fontSize: 15, fontWeight: FontWeight.w700)),
                          ),
                        ),
                      ],
                    ],
